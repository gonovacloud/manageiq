module ApplicationHelper
  ...
  def taskbar_in_header?
    if @show_taskbar.nil?
      ...
      if ! (@layout == "" && %w(auth_error change_tab show).include?(controller.action_name) ||
        %w(about chargeback ems_infra_dashboard exception infra_topology miq_ae_automate_button miq_ae_class miq_ae_export
  ...
  GTL_VIEW_LAYOUTS = %w(action availability_zone host_aggregate auth_key_pair_cloud
                        ...
                        infra_topology ems_network security_group floating_ip cloud_subnet network_router network_topology network_port cloud_network
                        load_balancer
