describe ApplicationHelper::Button::AuthKeyPairCloudCreate do
  describe '#disabled?' do
    it "when the create action is available then the button is not disabled" do
      ...
      ems = object_double(NOVAHawk::Providers::CloudManager.new, :class => NOVAHawk::Providers::Openstack::CloudManager)
      allow(NOVAHawk::Providers::Openstack::CloudManager::AuthKeyPair).to receive(:is_available?).and_return(true)
    ...
    it "when the create action is unavailable then the button is disabled" do
      ...
      button = described_class.new(view_context, {}, {}, {})
      ems = object_double(NOVAHawk::Providers::CloudManager.new, :class => NOVAHawk::Providers::Openstack::CloudManager)
      allow(NOVAHawk::Providers::Openstack::CloudManager::AuthKeyPair).to receive(:is_available?).and_return(false)
      allow(Rbac).to receive(:filtered).and_return([ems])
