describe ServiceAnsibleTower do
  ...
  describe '#launch_job' do
    it 'launches a job through ansible tower provider' do
      allow(NOVAHawk::Providers::AnsibleTower::ConfigurationManager::Job).to receive(:raw_create_stack) do |template, opts|
    ...
    it 'always saves options even when the manager fails to create a stack' do
      ...
      allow_any_instance_of(NOVAHawk::Providers::AnsibleTower::ConfigurationManager::Job).to receive(:stack_create).and_raise(provision_error, 'test failure')
