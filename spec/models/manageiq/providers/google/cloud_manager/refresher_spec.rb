...
describe NOVAHawk::Providers::Google::CloudManager::Refresher do
  before(:each) do
  ...
  def assert_specific_zone
    @zone = NOVAHawk::Providers::Google::CloudManager::AvailabilityZone.find_by_ems_ref("us-east1-b")
    expect(@zone).to have_attributes(
      :name   => "us-east1-b",
      :ems_id => @ems.id
    )

    @zone_central = NOVAHawk::Providers::Google::CloudManager::AvailabilityZone.find_by_ems_ref("us-central1-b")
  ...
  def assert_specific_key_pair
    ...
    @kp = NOVAHawk::Providers::Google::CloudManager::AuthKeyPair.where(:name => "root").first
    ...
    @project_kp = NOVAHawk::Providers::Google::CloudManager::AuthKeyPair.where(:name => "user2").first
  ...
  def assert_specific_flavor
    @flavor = NOVAHawk::Providers::Google::CloudManager::Flavor.where(:name => "f1-micro").first
  ...
  def assert_specific_custom_flavor
    custom_flavor = NOVAHawk::Providers::Google::CloudManager::Flavor.where(:name => "custom-1-2048").first
  ...
  def assert_specific_vm_powered_on
    v = NOVAHawk::Providers::Google::CloudManager::Vm.where(:name => "rhel7", :raw_power_state => "RUNNING").first
  ...
  def assert_specific_vm_with_proper_subnets
    v = NOVAHawk::Providers::Google::CloudManager::Vm.where(:name => "subnet-test", :raw_power_state => "RUNNING").first
    expect(v).to have_attributes(
      :template              => false,
  ...
  def assert_specific_vm_powered_off
    v = NOVAHawk::Providers::Google::CloudManager::Vm.where(
      :name            => "wheezy",
      :raw_power_state => "TERMINATED").first

    zone1 = NOVAHawk::Providers::Google::CloudManager::AvailabilityZone.where(:name => "us-central1-b").first
  ...
  def assert_specific_vm_preemptible
    v = NOVAHawk::Providers::Google::CloudManager::Vm.where(:name => "preemptible-1").first
    expect(v).to have_attributes(
  ...
  def assert_specific_image_template
    ...
    @template = NOVAHawk::Providers::Google::CloudManager::Template.where(:name => name).first
