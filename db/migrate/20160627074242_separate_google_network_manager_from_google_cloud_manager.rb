class SeparateGoogleNetworkManagerFromGoogleCloudManager < ActiveRecord::Migration[5.0]
  ...
  def up
    ...
    ExtManagementSystem
      ...
      .where(:ext_management_systems => {:type          => 'NOVAHawk::Providers::Google::CloudManager'},
      ...
      affected_classes.each do |network_model_class|
        network_model_class
          ...
          .update_all("type = 'NOVAHawk::Providers::Google::NetworkManager::#{network_model_class.name.demodulize}', ems_id = '#{network_manager.id}'")
      end
    end
  end
  ...
  def down
    ...
    ExtManagementSystem
      ...
      .where(:ext_management_systems => {:type => 'NOVAHawk::Providers::Google::CloudManager'}).each do |cloud_manager|
      network_manager = ExtManagementSystem.where(:parent_ems_id => cloud_manager.id).first
      affected_classes.each do |network_model_class|
        network_model_class
          .where(:ems_id => network_manager.id)
          .update_all("type = 'NOVAHawk::Providers::Google::CloudManager::#{network_model_class.name.demodulize}', ems_id = '#{cloud_manager.id}'")
      end
