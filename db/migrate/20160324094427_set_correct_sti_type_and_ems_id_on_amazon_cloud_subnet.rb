class SetCorrectStiTypeAndEmsIdOnAmazonCloudSubnet < ActiveRecord::Migration[5.0]
  ...
  def up
    ...
    ExtManagementSystem
      .where({:type => 'NOVAHawk::Providers::Amazon::NetworkManager'}).each do |ems|
      CloudSubnet
        ...
        .update_all("type = 'NOVAHawk::Providers::Amazon::NetworkManager::CloudSubnet', ems_id = '#{ems.id}'")
    end
  end
  ...
  def down
    ...
    CloudSubnet
      ...
      .where(:cloud_subnets => {:type => 'NOVAHawk::Providers::Amazon::NetworkManager::CloudSubnet'},
             :cloud_manager => {:type => 'NOVAHawk::Providers::Amazon::CloudManager'})
